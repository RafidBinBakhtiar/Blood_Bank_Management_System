...
#Fetch
$query = $db->prepare("SELECT * FROM recipient");
$query->execute();
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto mt-10">
        <h2 class="text-2xl font-bold text-center mb-6">Patient List</h2>
            ...
            <table class="min-w-full bg-white border border-gray-200">
                <thead>
                    <tr class="bg-gray-200 text-gray-700">
                        <th class="py-2 px-4 border">Patient ID</th>
                        <th class="py-2 px-4 border">Medical History</th>
                        <th class="py-2 px-4 border">Amount Needed (Bags)</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($patients as $patient): ?>
                    <tr class="text-center">
                        <td class="py-2 px-4 border"><?= htmlspecialchars($patient->recipient_id); ?></td>
                        <td class="py-2 px-4 border"><?= htmlspecialchars($patient->medical_history); ?></td>
                        <td class="py-2 px-4 border"><?= htmlspecialchars($patient->amount_required); ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
